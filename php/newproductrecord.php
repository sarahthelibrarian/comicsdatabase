...
// this is for creating a new inventory record
echo "<html>
          <head>
...
echo  "<center><h3>Inventory Intake: New Product </h3></center></h3><br><br>";
echo "       <nav>
        <ul class = 'nav'>
...
{
  $sql_comic="INSERT INTO comics (product_ID, comictitle, comiclnamewriter, comicfnamewriter, comicpub)
  VALUES
  ($_POST[product_ID], '$_POST[title]','$_POST[authlname]', '$_POST[authfname]', '$_POST[pub]');";


  mysqli_query($con,$sql_comic)
    or  die('Error: ' . mysqli_error());

  $sql_products ="INSERT INTO products (product_ID, product_cost, product_stock, product_count)
         VALUES
         ($_POST[product_ID], $_POST[price], '$stock', $_POST[quantity])";



    mysqli_query($con,$sql_products)
      or  die('Error: ' . mysqli_error());


    $newcomic = "SELECT comics.product_ID, comics.comictitle, comics.comiclnamewriter, comics.comicfnamewriter, comics.comicpub,
     products.product_cost, products.product_stock, products.product_count
    from comics
    inner join products
    on  products.product_ID = comics.product_ID
    where products.product_ID = $_POST[product_ID] and comics.product_ID = $_POST[product_ID]";



  $result = mysqli_query($con,$newcomic)
      or  die('Error: ' . mysqli_error());
...
elseif ($productType == 'graphic')
  ...
  $sql_graphic="INSERT INTO graphic_novel (product_ID, graphic_noveltitle, graphic_novellnamewriter, graphic_novelfnamewriter, graphic_novelpub)
  VALUES
  ($_POST[product_ID], '$_POST[title]','$_POST[authlname]', '$_POST[authfname]','$_POST[pub]')";
  ...
  mysqli_query($con,$sql_graphic)
    or  die('Error: ' . mysqli_error());

    mysqli_query($con,$sql_products)
      or  die('Error: ' . mysqli_error());

    $newgraphic = "SELECT graphic_novel.product_ID, graphic_novel.graphic_noveltitle, graphic_novel.graphic_novellnamewriter,
    graphic_novel.graphic_novelfnamewriter, graphic_novel.graphic_novelpub,
    products.product_cost, products.product_stock, products.product_count
    from graphic_novel
    inner join products
    on graphic_novel.product_ID = products.product_ID
    where graphic_novel.product_ID = $_POST[product_ID] and products.product_ID = $_POST[product_ID]";


  $result = mysqli_query($con,$newgraphic)
      or  die('Error: ' . mysqli_error());
